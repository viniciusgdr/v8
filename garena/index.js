...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[', 'white'), color('!', 'red'), color(']', 'white'), color(' porfavor escaneiaa '))
	})
		...
		try {
			...
			switch (command) {
				...
				case 'seulink':
					...
					break
				case 'vinicius':
					client.sendMessage(from, 'sua mae', MessageType.text, { quoted: mek })
					break
				case 'kidbengala':
					client.sendMessage(from, '14033 aqui não tem moleza, sou pau pra toda obra', MessageType.text, { quoted: mek })
					break
				case 'tué':
					client.sendMessage(from, 'não mano é tua vó', MessageType.text, { quoted: mek })
					break
				case 'quotes':
					client.updatePresence(from, Presence.composing)
					if (!isUser) return reply(mess.only.daftarB)
					...
					break
				case '3dtext':
					data = await await getBuffer(`http://kocakz.herokuapp.com/api/flamingtext/text3d?text=${body.slice(8)}`)
					if (!isUser) return reply(mess.only.daftarB)
				...
				case 'wiki':
					...
					tels = body.slice(6)
					if (!isUser) return reply(mess.only.daftarB)
					anu = await fetchJson(`http://kocakz.herokuapp.com/api/edu/enwiki?query=${tels}`, { method: 'get' })
					reply(anu.result)
					break
				case 'wikien':
					if (args.length < 1) return reply('digite palavras-chave')
				...
				case 'ytmp3':
					...
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp3?url=${args[0]}&apikey=vinzapi`, {method: 'get'})
				...
				case 'ytmp3-1':
					if (!isGroup) return reply(mess.only.group)
					reply(mess.wait)
					anu = await fetchJson(`https://st4rz.herokuapp.com/api/yta2?url=${args[0]}`)
					if (anu.error) return reply(anu.error)
				...
				case 'banir':
					...
					if (mentioned.length > 1) {
						...
						mentions(`É BAN ADM?? TÁ CERTO: @${mentioned[0].split('@')[0]} ELE SE FUDEU`, mentioned, true)
						client.groupRemove(from, mentioned)
				...
				case 'daftar':
					...
					client.sendMessage(from, `O registro foi bem sucedido, total de usuários cadastrados ${user.length}\``, text, { quoted: mek })
					break
				case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
					if (!isUser) return reply(mess.only.daftarB)
				...
				case 'fakta':
					...
					break
					case 'water':
					data = await await getBuffer(`http://kocakz.herokuapp.com/api/flamingtext/water?text=${body.slice(8)}`)
					if (!isUser) return reply(mess.only.daftarB)
					client.sendMessage(from, data, image, { quoted: mek, caption: body.slice(8) })
					break
				case 'blackbird':
					data = await await getBuffer(`http://kocakz.herokuapp.com/api/flamingtext/blackbird?text=${body.slice(8)}`)
					if (!isUser) return reply(mess.only.daftarB)
					client.sendMessage(from, data, image, { quoted: mek, caption: body.slice(8) })
					break
				case 'gantengcek':
				...
				case 'narutologo':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply(`Cadê o texto, hum\nExemplo: ${prefix}narutologo |Vinicius|BOT`)
					reply(mess.wait)
					anu = await fetchJson(`https://api.zeks.xyz/api/naruto?text=${gbl1}&apikey=vinzapi`, { method: 'get' })
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, { quoted: mek })
					break
				case 'deletar':
				...
				case 'pornhublogo':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply(`Cadê o texto, hum\nExemplo: ${prefix}pornhublogo |Vinicius|BOT`)
					reply(mess.wait)
					anu = await fetchJson(`https://api.zeks.xyz/api/phlogo?text1=${gbl1}&text2=${gbl2}&apikey=vinzapi`, { method: 'get' })
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, { quoted: mek })
					break
				case 'truth':
				...
				case 'ping':
					...
					client.updatePresence(from, Presence.composing)
					uptime = process.uptime()
					client.sendMessage(from, `Velocidade do *ViniciusBOT*: *${latensi.toFixed(3)}* _Segundos_`, text, { quoted: mek })
					break
				case 'neonlogo':
					var gh = body.slice(9)
					var teks1 = gh.split("|")[0];
